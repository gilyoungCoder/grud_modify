...
class GRUD(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, x_mean=0,\
                 bias=True, batch_first=False, bidirectional=False, dropout_type='mloss', dropout=0, device = 'cpu'):
        ...
        self.x_mean = torch.autograd.Variable(torch.tensor(x_mean)).to(self.device)
        self.bias = bias
    ...
    def forward(self, input):
        ...
        batch_size, _, _, num_steps = input.size()
        # 각 데이터 유형 분리
        ...
        for step in range(num_steps):
            ...
            gamma_h = torch.exp(-torch.max(self.zeros.to(self.device), (w_dg_h * d + b_dg_h)))
            ...
            if self.dropout == 0:
                ...
                z = torch.sigmoid((torch.matmul(x, w_xz) + torch.matmul(h,w_hz) + torch.matmul(m, w_mz) + b_z))
                r = torch.sigmoid((torch.matmul(x,w_xr) + torch.matmul(h, w_hr) + torch.matmul(m, w_mr) + b_r))
                h_tilde = torch.tanh((torch.matmul(x, w_xh) + torch.matmul((r * h), w_hh) + torch.matmul(m, w_mh) + b_h))
        ...
        output = torch.matmul(h, w_hy.t()) + b_y
